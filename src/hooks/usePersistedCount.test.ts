...
describe("usePersistedCount", () => {
	...
	it("resets to defaultCount when storageKey changes to a key with no stored value", () => {
		localStorage.setItem(
			"react-loaded",
			JSON.stringify({
				v: 1,
	...
	it("updates count when initialCount changes with storageKey", () => {
		const { result, rerender } = renderHook(
			(props: UsePersistedCountOptions) => usePersistedCount(props),
			{
				initialProps: {
	...
	it("prefers initialCount over localStorage on first render", () => {
		localStorage.setItem(
			"react-loaded",
			JSON.stringify({
				v: 1,
